import { hasInjectionContext, getCurrentInstance as getCurrentInstance$1, shallowReactive, reactive, effectScope, getCurrentScope, inject as inject$1, toRef, ref, isRef, isReactive, toRaw, onScopeDispose, watch, nextTick, toRefs, markRaw, computed, shallowRef, isReadonly, isShallow, defineComponent as defineComponent$1, createElementBlock, provide, cloneVNode, h, unref, Fragment, createVNode, Text, watchEffect, isVNode as isVNode$1, Comment, capitalize as capitalize$1, readonly, warn as warn$1, mergeProps, Transition, resolveDynamicComponent, withDirectives, openBlock, createElementVNode, onMounted, customRef, toValue as toValue$1, defineAsyncComponent, Suspense, createCommentVNode, withCtx, useSSRContext, createTextVNode, createBlock, onErrorCaptured, onServerPrefetch, createApp } from "vue";
import { $fetch as $fetch$1 } from "/Users/user/Documents/Code Project/microdemy-DS/node_modules/ofetch/dist/node.mjs";
...
import { START_LOCATION, createMemoryHistory, createRouter, isNavigationFailure, useRoute as useRoute$2, RouterView } from "vue-router";
...
const interpolatePath = (route2, match) => {
  return match.path.replace(ROUTE_KEY_PARENTHESES_RE$1, "$1").replace(ROUTE_KEY_SYMBOLS_RE$1, "$1").replace(ROUTE_KEY_NORMAL_RE$1, (r) => route2.params[r.slice(1)]?.toString() || "");
};
...
function getRouteBaseName(common, route2) {
  ...
  const routeName = isObject$2(_route) ? _route?.name : _route;
...
function getCanonicalQueryParams(common, ctx) {
  ...
  const canonicalQueries = isObject$2(ctx.seo) && ctx.seo?.canonicalQueries || [];
  ...
  const _props = new Proxy(props, {
    get(target, prop) {
      ...
      if (prop === "class" || prop === "style") {
        ...
        return componentDefaults.value?.[prop] !== void 0 ? componentDefaults.value?.[prop] : defaults2.value?.global?.[prop] !== void 0 ? defaults2.value?.global?.[prop] : propValue;
      }
      return propValue;
    }
  });
...
function useLink(props, attrs) {
  ...
  return {
    ...
    isActive: computed(() => {
      ...
      if (!route2.value) return link.value.isExactActive?.value ?? false;
      return link.value.isExactActive?.value && deepEqual(link.value.route.value.query, route2.value.query);
    }),
...
const injectionKeyIsVerticalNavHovered = /* @__PURE__ */ Symbol("isVerticalNavHovered");
const layoutConfig = {
  app: {
    title: "my-layout",
...
const isNavLinkActive = (link, _) => {
  ...
  if (!route2.meta.sidebarRoute)
    return false;
  return route2.meta.sidebarRoute === link.to?.name;
};
  ...
  setup(props, context) {
    ...
    if (props.shouldProvide) {
      provide(LayoutMetaSymbol, {
        ...
        isCurrent: (route2) => name === false || name === (route2.meta.layout ?? routeRulesMatcher(route2.path).appLayout ?? "default")
      });
    }
    const injectedRoute = inject$1(PageRouteSymbol);
    const isNotWithinNuxtPage = injectedRoute && injectedRoute === useRoute$1();
    const enclosingLayout = inject$1(LayoutMetaSymbol, null);
...
function markStableSlot(fn) {
  const wrapped = ((routeProps) => {
    ...
    if (result == null || !isVNode$1(result)) {
...
export {
  ...
  useRounded as A,
  useRouter as B,
  useProxiedModel as C,
  toPhysical as D,
  useLayoutItem as E,
  provideDefaults as F,
  useRender as G,
  VDefaultsProvider as H,
  propsFactory as I,
  makeThemeProps as J,
  makeTagProps as K,
  ...
  makeDisplayProps as O,
  makeComponentProps as P,
  ...
  LoaderSlot as a0,
  genOverlays as a1,
  makeVariantProps as a2,
  makeRouterProps as a3,
  makePositionProps as a4,
  makeLocationProps as a5,
  makeLoaderProps as a6,
  makeDimensionProps as a7,
  breakpoints as a8,
  EventProp as a9,
  useSize as aA,
  makeSizeProps as aB,
  useGroup as aC,
  useGoTo as aD,
  makeGroupProps as aE,
  useGroupItem as aF,
  makeGroupItemProps as aG,
  noop$1 as aH,
  defineFunctionalComponent as aI,
  consoleError as aJ,
  getObjectValueByPath as aK,
  isEmpty as aL,
  createRange as aM,
  keyValues as aN,
  ...
  consoleWarn as ae,
  getUid as af,
  getCurrentInstance as ag,
  only as ah,
  isOn as ai,
  pick as aj,
  filterInputAttrs as ak,
  callEvent as al,
  useResizeObserver as am,
  clamp as an,
  debounce as ao,
  ensureValidVNode as ap,
  matchesSelector as aq,
  omit as ar,
  getPropertyFromItem as as,
  isClickInsideElement as at,
  focusableChildren as au,
  focusChild as av,
  getNextElement as aw,
  defineComponent as ax,
  deprecate as ay,
  deepEqual as az,
  VProgressCircular as b,
  isObject$1 as b0,
  VuetifyLayoutKey as b1,
  refElement as b2,
