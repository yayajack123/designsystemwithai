...
import { a as VIcon, V as VBtn, b3 as VProgressLinear, aY as _export_sfc } from "../server.mjs";
...
const _sfc_main = /* @__PURE__ */ defineComponent({
  ...
  setup(__props) {
    ...
    const getSupportColor = (status) => supportStatusMap[status]?.color ?? "grey";
    const getSupportLabel = (status) => supportStatusMap[status]?.label ?? status;
    const getPaceLabel = (pace) => paceMap[pace]?.label ?? pace;
    const getRatingLabel = (rating) => ratingMap[rating]?.label ?? rating;
